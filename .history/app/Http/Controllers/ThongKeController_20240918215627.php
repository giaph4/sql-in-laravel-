...
class ThongKeController extends Controller
{
    ...
    public function thongKeDoanhThuTheLoai()
    {
        ...
        return response()->json([
            'data' => $data,
            'listTenTheLoai' => $listTenTheLoai,
            'listTongSoLuongBan' => $listTongSoLuongBan,
            'listTongThanhTien' => $listTongThanhTien
        ]);
    }

    public function tongChiTieuKhachHang()
    {
        $data = KhachHang::join('hoa_dons', 'khachhangs.id', 'hoa_dons.id_khach_hang')
                ->join('chi_tiet_hoa_dons', 'hoa_dons.id', 'chi_tiet_hoa_dons.id_hoa_don')
                ->select('ho_va_ten', DB::raw('SUM(chi_tiet_hoa_dons.thanh_tien) as tong_thanh_tien'))
                ->groupBy('ho_va_ten')
                ->get();
                $listHoVaTen = [];
                $listTongThanhTien = [];
                for ($i = 0; $i < count($data); $i++) {
                    $item = $data[$i];
                    $listHoVaTen[$i] = $item->ho_va_ten;
                    $listTongThanhTien[$i] = $item->tong_thanh
                    }
                    return response()->json([
                        'data' => $data,
                        'listHoVaTen' => $listHoVaTen,
                        'listTongThanhTien' => $listTongThanhTien
