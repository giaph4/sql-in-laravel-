...
class ChiTietThongKeController extends Controller
{
    ...
    public function chiTietThongKeSachNhapKho(Request $request)
    {
        $data = Sach::where('ten_sach', $request->ten_sach)
            ...
            ->join('phieu_nhaps', 'chi_tiet_phieu_nhaps.id_phieu_nhap', 'phieu_nhaps.id')
            ->select('sachs.*', 'chi_tiet_phieu_nhaps.*', 'phieu_nhaps.*')
            ->get();
        return response()->json(['data' => $data]);
    }

    public function chiTietThongKeDoanhThuSach(Request $request)
    {
        $data = Sach::where('ten_sach', $request->ten_sach)
            ->join('chi_tiet_hoa_dons', 'sachs.id', 'chi_tiet_hoa_dons.id_sach')
            ->join('hoa_dons', 'chi_tiet_hoa_dons.id_hoa_don', 'hoa_dons.id')
            ->select('sachs.*', 'chi_tiet_hoa_dons.*', 'hoa_dons.*')
            ->get();
        return response()->json(['data' => $data]);
    }

    public function chiTietThongKeDoanhThuTheLoai(Request $request)
    {
        $data = TheLoai::where('ten_the_loai', $request->ten_the_loai)
            ->join('sachs', 'the_loais.id', 'sachs.id_the_loai')
            ->join('chi_tiet_hoa_dons', 'sachs.id', 'chi_tiet_hoa_dons.id_sach')
            ->join('hoa_dons', 'chi_tiet_hoa_dons.id_hoa_don', 'hoa_dons.id')
            ->select('sachs.*', 'chi_tiet_hoa_dons.*', 'hoa_dons.*', 'the_loais.*')
            ->get();
        return response()->json(['data' => $data]);
    }
}
